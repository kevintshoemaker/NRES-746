...
paramslist <- lapply(1:nrow(LLsurface), function(t){MLE['shape']=LLsurface$shape[t];MLE['rate']=LLsurface$rate[t];MLE } )
LLsurface$LL <- sapply(paramslist, function(t) -GammaNLL(t))   # note minus sign to turn NLL to LL 
...
Clearly this is the most inefficient, brute-force method we have seen so far (aside from the actual brute force method).
