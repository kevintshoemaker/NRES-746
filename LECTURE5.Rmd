...
For some reason (which we will not dwell on) the deviance under the null hypothesis (the full model has no useful information that the restricted model doesn't already have) is approximately $\chi^2 $ ("chi-squared") distributed with *r* degrees of freedom, where *r* is the number of dimensions by which the full model has been reduced.  
...
Okay, so now we know that the deviance, the quantity defined as $-2*ln(\frac{\mathcal{L} _{r}}{\widehat{\mathcal{L}}})$, should be distributed according to the above probability distribution if we are "fixing" a single free parameter in our model. How does this relate to confidence intervals?
