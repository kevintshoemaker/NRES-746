...
Please sign up (on our shared Google Sheets) for a time slot (20 mins) to discuss your group project proposals with me tomorrow. The rest of lab time is yours to either work on the group projects or get started with lab 3 (if I finish revising it in time!)  
...
Clearly, we need optimization algorithms!! In addition, they provide an excellent example of how computers (often via brute force algorithms) can supersede pure mathematics for performing statistical analysis. 
...
Note that we won't be building optimization algorithms in class- the code in this lecture is for demonstration purposes only!
...
NOTE: "optim()" doesn't work that well for optimizing single parameters (R recommends using "optimize()"), but in practice we will mostly be optimizing across multiple dimensions and "optim()" is far more general and powerful then "optimize()".
...
But as dangerous ecological statisticians we aren't satisfied with using a "black box" like the "optim()" function, we need to understand what is going on behind the scenes. Maybe someday we'll want to write our own optimizer!  

Let's start with the conceptually simple, often computationally impossible, brute force method... 
...
If we assume that the likelihood surface is smooth (differentiable) and has only one minimum, we can develop very efficient optimization algorithms. In general, derivative-based methods look for the point in parameter space where the derivative of the likelihood function is zero. That is, at the peak!  
...
Now let's develop a function to implement our first move through parameter space, according to the rules defined above... 
...
In general, the simplex-based methods are very good at finding the general vicinity of the MLE, but generally less efficient than the derivative-based methods- especially as you near the MLE.   
...
3. Evaluate curvature at the MLE and use that to estimate error (somewhat inexact but efficient- but generally performs pretty well, and is the default for many MLE routines!)
