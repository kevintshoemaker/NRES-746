...
But wherever there is randomness (stochasticity), we can get different results every time (that's what it means to be random!). In such cases, a single output of the data generating model by itself has little meaning. However, we can extract a great deal of meaning if we run lots of replicates. _*The distribution of data across all replicates becomes the real result!*_ 
