...
Your data simulation model could be as simple as the random numbers you were just generating. For example, we could assume that our sample data were generated from a Poisson process with constant mean ($\lambda$). 
...
Now, let's look at the **stochastic component** (also known as the "noise"). Recall that a normal distribution is defined by a mean and a variance (or standard deviation). We can consider the deterministic component as representing the **mean** (expected) value of the response for any given value(s) of relevant covariates. Therefore, all we need to generate stochastic data for any given covariate value(s) is a **variance**, or standard deviation. 
...
Is the following linear regression model a good fit to the data?
