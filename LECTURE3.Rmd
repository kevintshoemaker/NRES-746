...
**A**: Well, it depends on what flavor of t-test. The simplest model could be something like this: The data from sample A and sample B are drawn from the same normally distributed data population (same mean and same variance). [this is a one-sided, two sample t-test with equal variance].
...
You can think of the deterministic component as the **"signal"** and the stochastic component as the **"noise"**. Most data comprise signal and noise.
...
How well does this model fit the data? Is this model likely to produce these data? (we will revisit this concept more quantitatively when we get to likelihood-based model fitting!)
