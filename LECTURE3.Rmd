...
We can evaluate this question by brute force programming. And we will, because we love brute-force programming!
...
**NOTE**: Frequentist statistical tests are based on a single sample that is one of an theoretically infinite number of potential samples. The interpretation of the results is implicitly based on the idea of sample replication (e.g., "if the null hypothesis were true, and the experiment were *replicated* lots and lots of times, results as or more extreme as the observed results could be expected from x% of replicates")
...
In such cases, probably the most straightforward (yet not always the most efficient) way to address these questions is to simulate data under various sampling strategies and signal/noise ratios, and see how well we can recover the "true" signal through the noise!
...
* A single surveyor has a 2% chance of detecting each animal in the population in a single day of surveying     
