...
Hopefully it is clear that the Metropolis-Hastings MCMC method could be modified to fit arbitrary numbers of free parameters for arbitrary models. However, the M-H algorithm is not necessarily the most efficient or the most general of MCMC algorithms. That award tends to go to the **Gibbs sampler**. In lab we will play around with Gibbs samplers, mostly using an amazing piece of software called **BUGS** (**B**ayesian **I**nference **U**sing **G**ibbs **S**ampling).
...
In many cases, we can't work out the full posterior distribution for our model directly, but we **CAN** work out the conditional posterior distribution if all parameters except for the parameter in question were known with certainty, the posterior distribution is generally fairly easy to work out!
...
Then we can "package" the data into a single list object that contains all the relevant data referenced in the BUGS code:
...
One simple and intuitive convergence diagnostic is the *Gelman-Rubin diagnostic*, which assesses whether chains are more different from one another than would be expected on the basis of simple Monte Carlo error:

```{r}
gelman.diag(jagsfit.mcmc)
```

In general, values of 1.2 or higher are considered poorly converged. If so, you should try running longer chains!
