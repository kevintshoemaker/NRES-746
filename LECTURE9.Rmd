...
[Click here](LECTURE9.R) for the companion R script for this lecture.
...
3. **Generality**: The model performs well at extrapolating responses for out-of-sample data that are outside the range of those in the training set (the model is really useful for prediction!)

NOTE: if we are doing causal inference, the model might be "good" if it helps us estimate the causal effect of A on B, regardless if it makes good predictions... 
...
Now we'll use ML to fit the Ricker model with a gamma error distribution.
...
Here's the stan code for the basic myxomatosis/gamma case:
...
Clearly the model performance is lower under cross-validation, and almost always will be! But this is also a more honest evaluation of model performance. 
