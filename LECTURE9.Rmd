...
1. **Goodness-of-fit**: The data could easily/reasonably have been generated under the best-fit model (the model is plausible)
2. **Predictive performance**: The model performs well at predicting observed responses (the model is useful) 
3. **Generality**: The model performs well at extrapolating responses for data that are qualitatively different from those in the training set (the model is really useful!)
...
Often, we compare a target summary statistic (e.g., Deviance, RMSE) for the observed data with the range of that target summary statistic produced under the inferred, stochastic data generating model.   
...
In many cases, goodness-of-fit is not ultimately what we are most interested in. What we really want to know is whether the model does a good job at predicting the response variable. Is our model useful in some way? 
...
So the average error in units of titer (in log10 rabbit infectious doses) (whatever that means!)       

So is our model *good*???  

Does it really do a good job at prediction? 
