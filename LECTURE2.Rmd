...
Here we use the **law of total probability**, which is just a restatement of the third axiom: the total or unconditional probability of an event is the sum of all the disjoint experimental outcomes that comprise the event. In this case, the unconditional probability of drawing a blue item can be expressed:
...
Suppose the infection rate (prevalence) for a rare disease is one in 10,000:
...
How can we use these components to compute what we want: $PPV = P(+|test_{+})$? 
...
This formula is at the core of **Bayesian statistics**

The big idea is that the evidence doesn't tell us what we need in a vacuum. The evidence just allows us to **update our prior beliefs**. 
...
Under this paradigm, truth is hidden behind a veil of sampling variability. If we had perfect knowledge (infinite sample size) we would know the answers we seek. The only thing that prevents certain knowledge is that we can't observe everything- instead we must rely on a small sample from the population of interest. The unknowable truth is fixed- it is absolutely NOT a random variable. However, we can account for sampling variance by thinking of our sample data as one out of many possible samples. We treat our sample as a set of random variables, so that any statistics derived from our sample are also random variables with some sampling distribution. By imagining our sample as one of infinity possible samples, we can account for sampling error, and thereby quantify the uncertainty and reliability of our results. For example, we may reject a "no-effect" null hypothesis if our data are inconsistent with random sampling variability under this null hypothesis (this is the ubiquitous p-value!).      

#### What is Bayesianism
Bayesian inference allows us to describe our uncertainty about parameters using the language of probability - that is, the truth value of a hypothesis can be modeled as a random variable! For example, if we fit a linear regression model, Bayesian statistics can tell us the degree to which we believe that a regression coefficient (e.g., $\beta$) is greater than zero because the coefficient itself has a probability distribution associated with it (called the "posterior distribution"). In Bayesian statistics, probability is interpreted as a _degree of belief_ about the truth (e.g., the true value of a regression coefficient, or the degree of believe to place on a particular hypothesis). That said, Bayesian analyses require specifying a *prior probability* on all statements of truth (what we know before we collect any data). Bayesian inference is all about updating your belief- if you don't start with a prior belief, there is nothing to update, and the Bayesian way of thinking breaks down. But what if you don't have any prior knowledge about a parameter? 

### Which paradigm is better?
We will not delve into the long-standing debate between the Bayesian and frequentist paradigms, but I encourage you to read more about this: for example, in the excellent volume "The Nature of Scientific Evidence" edited by Taper and Lele (which veers toward frequentism) and "Bernoulli's Fallacy" (by Aubrey Clayton) which makes a strong case for Bayesianism. 

The pragmatic analyst admits that they are both useful in different contexts, and uses both paradigms freely!! 
