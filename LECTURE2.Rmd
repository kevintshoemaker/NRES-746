...
We then define an **event**, which is a subset of the sample space- events are what we can ultimately assign a probability to. 
...
Finally, we define a **probability** which assigns a number between 0 and 1 to any measurable event. 

A **probability space** combines the whole trio: sample space, event space, and probability measure:$\{\Omega,F,P\}$.

When the sample space (and events within this space) is mapped to the real number line, $\mathbb{R}$, then we call the mapping a **random variable** (often denoted with a capital letter, like X). Think of this as a placeholder for any possible numeric outcome of a random trial - like a coin that could come up heads (e.g, 0) or tails (e.g., 1). 
