...
In the last class (and lab), we talked about simulating data from models. This is often called *forward* modeling (or prediction)- that is, we take a model and use it to predict emergent patterns. The process flow goes something like this:

$Model \rightarrow Data$

In this class, we will talk about *inference* using the method of **maximum likelihood**. Inference is in some ways the opposite process; we are using the data to say something about the model. 

$Data \rightarrow Model$

In many ways, these two processes -- *forward simulation* and *inference* -- are interrelated. Simulation can help us to make better inferences, and inference can help us to construct better simulation models!

First let's see how simulation can help us to make inferences. This leads directly into the core idea of maximum likelihood!
...
The **algorithm** is:
+ A data generating model, often constructed by combining one or more deterministic and stochastic components. Likelihood is computed by adding together the log-likelihoods of all observations.        

The **output** is:    
+ The likelihood of the dataset, given the model     
