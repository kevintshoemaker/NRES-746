...
Here is the "real" lab exercise for this week! For the assignment, please keep your work as short and sweet as possible! **LAB 2 is due by midnight on Friday 9/30.** Comment each line of your code. Use functions wherever possible. Don't be afraid of for loops. And be dangerous!
...
* Does the 95% confidence interval around the parameter estimate (estimated from the data) contain the true parameter value 95% of the time (a frequentist question!). Show this with simulations?
...
* Plot the estimated regression line with a 95% prediction interval. Overlay the "true" 95% prediction interval (e.g., using the "quantile" function). 
...
* Extra credit: Can you write code to simulate the range of regression lines likely to be produced under the fitted model (i.e, within the 95% confidence interval)? Use this code to plot out a large number of replicate regression lines. Overlay the 95% confidence interval produced using the "predict" function. If you can do this, you should be able to write code to simulate the range of data that could reasonably be produced under the fitted model (i.e, simulate data using the fitted parameter estimates and confidence intervals)? Use this code to plot out a large number of replicate regression lines. Overlay the 95% confidence interval produced using the "predict" function. 

#### QUESTION 2 part b (model verification #2)
Write an R function to generate data that violates *one or more* of the basic assumptions regarding the error distribution of standard linear regression. Make sure your dataset is realistic! Justify in writing how biological or sampling processes could have produced the data, including (especially) the assumptions violation(s). NOTE: you must specify a linear relationship (y=mx+b) between your response and predictor (otherwise some of the questions below wouldn't make sense!). 
