...
For those wishing to follow along with the R-based demo in class, [click here](RandomForest2.R) for the companion R script for this lecture.

Follow along with our powerpoint presentation [here](RandomForest2.pptx)   
...
df <- read.csv("Post1_Merge_Database2.csv")
...
n.folds <- 5
df$ID
uniquedeer <- as.character(unique(df$ID))
folds_df <- data.frame(
  deer = uniquedeer,
  fold = c(rep(1:n.folds,each=5),1,2)
)

foldVector <- folds_df$fold[match(as.character(df$ID),folds_df$deer)]
