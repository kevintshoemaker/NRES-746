...
DrawLine(trees$Height,trees$Volume)    # run your new function!
...
DrawLine2(x=trees$Height,y=trees$Volume,smooth=T,span=10)    # run your new function!
...
CLTdemo <- function(n.samples=1000,sample.size=10,min=10,max=20,plot=TRUE){
...
CLTdemo(n.samples=4000,sample.size=5,min=10,max=20)    # run your new function!
...
BootCoefs <- function(df=trees,statfunc=RegressionCoefs,n_samples=1000,n_stats=2,responsevar="Volume"){    # univariate models only- interaction and multiple regression not implemented here
  boot <- boot_sample(df=df,statfunc=statfunc,n_samples=n_samples,n_stats=ncol(df)-1)   # generate test statistics (Rsquared vals) for 1000 bootstrap samples
  confint <- apply(boot,2,function(t)  quantile(t,c(0.025,0.5,0.975)))       # summarize the quantiles to generate confidence intervals for each predictor variable
  colnames(confint) <- paste("stat",c(1,2),sep="")
  return(t(confint))     
}
