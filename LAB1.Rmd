...
1.  Review the meaning of the [Central Limit Theorem](https://en.wikipedia.org/wiki/Central_limit_theorem), which implies that the mean of a sufficiently large number of independent random variables will have a *sampling distribution* that is approximately normally distributed.     
...
4.  The following code for illustrating the Central Limit Theorem was inspired by Teetor, Paul. 2011. R Cookbook. O-Reilly Media, Inc. (p. 45), with modification. Type (or paste) the following code into your R script window (or Rstudio script window):    
