...
a. Review the meaning of the Central Limit Theorem, which states that the mean of a sufficiently large number of independent random variables will be approximately normally distributed.    
b. Open the R script window from the File menu.   
c. The following code for illustrating the central limit theorem was inspired by Teetor, Paul. 2011. R Cookbook. O-Reilly Media, Inc. (p. 45), with substantial modification. Type (or paste) this into your R script window (or Rstudio script window):  

```{r results='hide'}
    # define the true distribution
    # define the true distribution (for now, uniform)
...
TRUEMAX <- 20      #  ... and the maximum
   ...
   # define the samples to be drawn from this hypothetical uniform distribution
N_IND_SAMPLES <- 1000   # number of random samples to draw
SAMPLESIZE <- 10       # size of each sample

@@ -51,31 +57,63 @@ for(i in 1:N_IND_SAMPLES){
...
(1) copy and paste from the script window into the console;     
(2) use <ctrl R> key to execute line by line from within the script window;     
(3) use <ctrl A> to select the whole code block, then <ctrl R> to execute all at once;     
(4) save the script to a text file with .R extension using File…Save from menu, and then run the script using the source() function, e.g. source("H:\\Courses\\NRES746\\ Lab1_Aug 30\\CentralLimitTheorem.R")    
...
g. The # sign used above allows the R programmer to insert comments adjacent to snippets of code, which facilitates readability of code (and lets the programmer remember later on what he/she was thinking when coding things a certain way!). To show the instructor that you understand all code in this short script, comment every line so as to describe its precise meaning, including all variables and functions. To accomplish this you will probably need to consult the R help documentation. A simple way to achiev...(line truncated)...
...
So can you conclude that the sample means came from a normal distribution?

#### QUESTION 2 
The # sign used above allows the R programmer to insert comments adjacent to snippets of code, which facilitates readability of code (and lets the programmer remember later on what he/she was thinking when coding things a certain way!). To show the instructor that you understand all code in this short script, comment every line so as to describe its precise meaning, including all variables and functions. To accomplish this you will probably need to consult the R help documentation. The commented code for th...(line truncated)...
...
#### QUESTION 4.
Determine a reasonable rule for how large a sample size is necessary to ensure that the sample mean follows a normal distribution (characterized by variance of $sigma/n$]) regardless of the distribution of the data.    

---------------------------
###VI. 
Multiple Regression Analysis, Exercise One: Air Quality Data  
