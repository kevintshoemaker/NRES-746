...
d <- group_by(gambia, x, y) %>%
  summarize(
    total = n(),
    positive = sum(pos),
    prev = positive / total    # prevalence
...
As a wizard geostatistical epidemiologist working for the government, you know that when prevalence of malaria exceeds 20% on a local scale, it tends to spread to nearby villages at much higher rates? You want to know what is the probability that prevalence is at or above 20% anywhere in the region, so that you can inform your team where the highest risk areas are to send public health response teams and focus actions for prevention. You can do this.   
...
1 - inla.pmarginal(q = threshold, marginal = marg)
...
Just pick one or two of the questions, whatever appeals to you, and write your comments in a word document to turn in. Alternatively, if something during the demo sparked your interest that you want to follow up on, go for that instead, and just write some sentences about what you did and what you found out.
...
4. Using exceedence probabilities to inform decisions. What are 
