...
In the previous two lectures, we have discussed **model-based inference** using likelihood functions in a *frequentist* framework. Specifically, we have constructed likelihood functions with one or more free parameters and then used numerical optimization to find the maximum-likelihood estimate (MLE). Parameter uncertainty was accommodated by assessing the curvature of the likelihood surface in the vicinity of the MLE (e.g., using the 'rule of 2' and the concept of the *likelihood profile*). 

Today we will start discussing **Bayesian inference**, which is less of a departure from these previous topics than you might think. 
