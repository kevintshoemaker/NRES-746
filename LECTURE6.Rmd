...
We quantified parameter uncertainty by relying on the asymptotic normality property of ML estimators, and by leaning on Wilks' theorem, which allows us to treat the deviance statistic (-2 * log likelihood ratio) as following a Chi-squared distribution under the null hypothesis of no difference. Regardless of the mathematical details, the key idea was that the curvature of the likelihood surface in the vicinity of the MLE (e.g., the *likelihood profile*) tells us something about how certain we are about our ML estimates. 
...
The difference is that we are no longer interested as much in the maximum likelihood estimate (MLE) or the asymptotic properties of maximum likelihood estimators. Instead, we're interested in using the likelihood function to update our **degree of belief** about our model and its parameter values. Effectively, we want to obtain a probability distribution (called the **joint posterior**) that reflects our beliefs about the plausibility of any given set of parameters.

Remember, Bayesian inference allows us to represent parameter uncertainty as probability distributions, so the parameters of interest are no longer fixed but unknown values (point estimates), they are sets of potential values, each with an associated probability.
...
A conjugate prior is a distribution for a parameter that matches the data-generating model: that is, the prior has the same general form as the likelihood function. An important consequence of this is that prior and the posterior distribution can be represented with the same probability distribution (e.g., gamma, beta, normal). We will come back to this!
...
For continuity we will continue the myxomatosis dataset. Recall that we are estimating the shape and scale (or alternatively, the rate) parameters of the Gamma distribution that describes the virus titer in Australian rabbits for the highest-grade infections. 
