...
Imagine we conduct a tadpole experiment, where two of ten tadpoles die. We are interested in comparing a simple model where *p* is fixed at 0.5 with a more complex model where *p* is a *free parameter*.  
...
**Q** What is the *marginal likelihood* under the simple model given an observation of 2 mortalities?

Now lets' think about the more complex model, where *p* is a free parameter. First, let's assume that *p* is assigned a uniform $beta(1,1)$ prior across parameter space:

```{r}
curve(dbeta(x,1,1))
```

What is the marginal likelihood of observing any particular number of mortalities from 0 to 10? Intuitively, because *p* could take any possible value with equal probability, we know that the marginal likelihood of observing 2 mortalities should be the same as making any other observation. That is, neither the likelihood function nor the prior distribution favors any particular observation over any other. 


We can prove this to ourselved with a simple experiment:
...
Therefore, since there are 11 possibilities, the marginal likelihood of any particular data observation under the model with p as a free parameter should be 1/11.
...
The simpler model wins!!! That would never happen if we looked at raw deviance- the deviance of the more highly-parameterized model will always be lower. That's why we have to penalize or *regularize* more highly-parameterized models.  

Does it make sense that the principle of parsimony is naturally incorporated in the Bayes factor? That is, we don't need to impose a penalty term- it is there already!!!


### Deviance Information Criterion (DIC)

DIC is computed by default in JAGS and WinBUGS. And it is perfectly analogous to other I-T metrics like AIC (and therefore easy to interpret and use)!

... but it is generally not reliable and I would not reccommend using it in most cases... 
...
Another cool, but not perfect, method of Bayesian model selection is to write the model selection directly into the JAGS code!
