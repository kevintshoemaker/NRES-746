...
One additional point of complexity in this data set- some trees were sampled from areas that have undergone periodic wave-like die-offs (recruitment of balsam fir tends to occur after periodic die-offs, resulting in stands with most trees coming from the same cohort). Other trees were sampled from areas that have not undergone recent die-offs. 
...
Let's compute the *-2 times the log likelihood* ($-2*log(likelihood)$) of the two models at the MLE (sometimes confusingly called the "deviance"). That is, we compute $-2*log(Lik@MLE)$ for each alternative model.
...
Because the raw likelihood ratio under the null hypothesis does not have a known distribution, we first convert the likelihood ratio to a **deviance**:
...
When expressed as a deviance, the likelihood ratio (asymptotically) should be approximately Chi-squared distributed with df equal to the number of fixed dimensions.
 
The LRT can be used for *two-way model comparison* as long as one model is nested within the other (full model vs. reduced model). If the models are not nested then the LRT doesn't really apply.
...
A common *rule of thumb* is that models within 2 AIC units of the best model are "reasonable" (does this "rule of 2" sound familiar?)
